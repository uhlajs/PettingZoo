...
* Many classic environments have illegal moves in the action space, and describe legal moves in  `env.infos[agent]['legal_moves']`. In environments that use this, taking an illegal move will give a reward of -1 to the illegally moving player and 0 to the other players. Game will continue with the player having another chance to make their move. Note that this list is only well defined right before the agents takes its step.
